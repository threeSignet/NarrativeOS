...
### 模板参考（仅展示层级结构，禁止复用其中的名称）
以下模板展示的是**真实的空间层级结构**，不是概念分类。括号中的名称仅为结构示意，你必须为小说创作全新的名称：
- 都市灵异/东方玄幻：世界体系 → 大域界 → 大区域 → 城市/聚落 → 片区 → scene
  ❌ 错误示例：世界体系 → 维度界域（按存在形态划分）→ ...  ← "维度界域"是概念不是地理
...
- **必须根据小说的【类型】【简介】【核心创意】创作全新的地名，禁止复用本提示词中的任何示例名称**

## 地理命名规范（必须遵守）
- 空间域名必须是**纯地名**，像真实世界中的正式地名
- **禁止**加前缀标签（如 前缀·地名）
- **禁止**加功能描述前缀（如 功能区·地名）
- **禁止**游戏化命名（如 XX禁区·XX遗址）
- **禁止**概念化命名（如 XX维度层、XX渗透亚空间）
- 宏观层级用正式大名称，微观层级用具体场所名
- 世界观特殊属性（阴阳属性、灵气浓度、区域功能等）放在 content 字段内描述，**不放在名称中**
- **禁止复用本提示词中的示例名称（如西方奇幻的"创世之柱"等），必须根据小说设定创作全新地名**
  ...
  "proposals": [
    {
      ...
      "content": {
        ...
        "payload": {
          ...
          "items": [
            { "subtype": "region", "name": "（原创纯地名）", "summary": "一句话概括", "content": { "scale": "（第一级尺度的 key）", "coordinates": { "x": 500, "y": 400 }, "climate": "气候", "terrain": "地形", "resources": "资源", "cultural_significance": "定位", "needs_refinement": true } }
          ],
          "relations": [
...
- 子条目应该是对应粒度定义中的"典型单元"级别的真实空间
- **判断标准**：每个子条目的空间规模必须与「${targetLabel}」粒度定义中的"典型单元"一致
- 禁止使用"维度层""存在形态""灵异渗透层"等概念性命名——这些不是地理名称
...
- **必须根据小说的具体设定创作全新的、与小说风格匹配的地名，禁止复用本提示词中的任何示例名称**
  ...
  "proposals": [
    {
      ...
      "content": {
        ...
        "payload": {
          ...
          "items": [
            { "subtype": "region", "name": "（原创纯地名）", "summary": "一句话概括", "content": { "scale": "${targetScale}", "coordinates": { "x": 480, "y": 120 }, "parentName": "${ref.parentName}", "climate": "气候", "terrain": "地形", "resources": "资源", "cultural_significance": "定位", "needs_refinement": true } }
          ],
          "relations": [
